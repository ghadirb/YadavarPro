...
| `BAZAAR_IAB_PUBLIC_KEY` | **الزامی برای خرید درون‌برنامه‌ای بازار** — کلید RSA عمومی از پنل بازار |
| `MYKET_IAB_PUBLIC_KEY` | **الزامی برای خرید درون‌برنامه‌ای مایکت** — کلید RSA عمومی از پنل مایکت |

اگر این Secretها از قبل ست شده‌اند، دوباره نساز. عوض کردنشان یعنی **کلید جدید** و شکست انتشار.

⚠️ این دو مقدار کلید امضای برنامه نیستند و ربطی به `RELEASE_KEYSTORE_*` ندارند. همچنین با
`MYKET_ACCESS_TOKEN` (که در سرور/Apps Script برای تایید رسید خرید استفاده می‌شود) اشتباه
گرفته نشوند - `MYKET_IAB_PUBLIC_KEY` مقداری است که کتابخانه `myket-billing-client` **داخل
خود اپلیکیشن** برای برقراری اتصال اولیه به فروشگاه مایکت لازم دارد. اگر این Secret خالی
باشد، بیلد بدون خطا انجام می‌شود ولی در گوشی کاربر با پیام «اتصال به فروشگاه برقرار نشد»
مواجه می‌شوید - دقیقاً همان خطایی که در بررسی مایکت گزارش شد. از نسخهٔ فعلی به بعد،
`build.gradle.kts` این حالت را برای بیلدهای Release به‌صورت خودکار fail می‌کند تا این اشتباه
دوباره بی‌صدا رخ ندهد.

کلید RSA عمومی مایکت را از پنل توسعه‌دهندهٔ مایکت (developer.myket.ir) → برنامهٔ خود →
بخش «پرداخت درون‌برنامه‌ای» یا «تنظیمات API» بردارید (معمولاً با عنوان «کلید عمومی RSA»
یا «Public Key» مشخص شده است) و همان رشتهٔ کامل (بدون فاصله یا خط جدید اضافه) را در
Secret گیت‌هاب قرار دهید.
